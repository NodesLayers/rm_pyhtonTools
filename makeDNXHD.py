...
# Path to FFmpeg
ffmpeg = os.path.normpath("Y:\\dng02_mae\\__PIPELINE\\software\\ffmpeg-20170628-c1d1274-win64-static\\bin\\ffmpeg.exe")
...
if ffTest != "mov":    
    ...
    outputFile = os.path.basename(filePath)
    outputFile = outputFile.split(".")
    outputFile = outputFile[0]
    outputFile = os.path.join(os.path.dirname(filePath),outputFile+"_dnxHD.mov")
    print("Output File:"+outputFile)
    
    
    # Create CMD call
    cmd = ffmpeg+" -y -i "+filePath+" -c:v dnxhd -vf scale=1920x1080,fps=23976/1001,format=yuv422p -b:v 115M "+outputFile
